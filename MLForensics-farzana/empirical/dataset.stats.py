...
def getDevEmailForCommit(repo_path_param, hash_):
    ...
    author_emails = [x_.replace(hash_, '')
    ...
    if os.path.exists(full_path_to_repo):
        try:
            ...
            try:
                ...
                print('Skipping this repo ... due to branch name problem',
                      full_path_to_repo)
        except (exc.GitCommandError, exc.InvalidGitRepositoryError):
        ...
        for commit_ in all_commits:
            ...
            str_time_commit = timestamp_commit.strftime(
                '%Y-%m-%d')  # date with time
            all_time_list.append(str_time_commit)

    else:
        repo_emails = [str(x_) for x_ in range(10)]
    all_day_list = [datetime(2020, 11, 20, 00, 15)]
    all_day_list = all_day_list + [datetime(int(x_.split('-')[0]), int(
        x_.split('-')[1]), int(x_.split('-')[2]), 12, 30) for x_ in all_time_list]
...
def getAllCommits(all_repos):
    ...
    for repo_ in all_repos:
        ...
        the_tuple = (repo_, dev_cnt, com_cnt, per_repo_min_day,
                     per_repo_max_day, day_diff)
    ...
    temp_df.to_csv('COMMIT.STATS.csv', header=[
                   'REPO', 'DEVS', 'COMMITS', 'START_DATE', 'END_DATE', 'DURATION_DAYS'], index=False, encoding='utf-8')
        ...
        if 'ZOO' in result_file:
            ...
            for temp_dir in temp_dirs:
                list_subfolders_with_paths = [
                    f.path for f in os.scandir(temp_dir) if f.is_dir()]
        ...
        for full_path_to_repo in all_repos:
            ...
            if os.path.exists(full_path_to_repo):
                ...
                try:
                    ...
                    print(
                        'Skipping this repo ... due to branch name problem', full_path_to_repo)
                for commit_ in all_commits:
                    commit_hash = commit_.hexsha
                    emails = getDevEmailForCommit(
                        full_path_to_repo, commit_hash)
                    repo_emails = repo_emails + emails
            else:
                repo_emails = [str(x_) for x_ in range(10)]
    repo_emails = np.unique(repo_emails)
